...
# --- 2. CUSTOMER DIRECTORY ---
    ...
    with tab1:
        ...
        with st.form("register_customer_form_alt"):
            col_a, col_b = st.columns(2)
            with col_a:
                name = st.text_input("Name (Required)", placeholder="John Doe", key="alt_reg_name")
                email = st.text_input("Email Address (Required)", placeholder="john@example.com", key="alt_reg_email")
                phone = st.text_input("Phone Number", placeholder="+1 (555) 0123", key="alt_reg_phone")
            with col_b:
                license_no = st.text_input("Driver License No.", placeholder="DL-XXXXXX", key="alt_reg_dl")
                address = st.text_area("Billing Address", placeholder="Street, City, State, ZIP", key="alt_reg_address")
    ...
    with tab2:
        ...
        if st.button("🔄 Refresh Customer List", key="alt_refresh_cust"):
            st.rerun()
    ...
    with tab3:
        ...
        search_id = st.text_input("Enter Customer Database ID", key="alt_search_cust_id")
        if search_id and st.button("Fetch Profile", key="alt_fetch_profile"):
    ...
    with tab4:
        ...
        if isinstance(customers, list) and customers:
            ...
            selected_c_label = st.selectbox("Select Customer to Update", list(customer_options.keys()), key="alt_select_cust_up")
            selected_c = customer_options[selected_c_label]
            
            with st.form("update_customer_form_alt"):
                st.markdown(f"Updating records for Customer ID: **{selected_c['id']}**")
                up_name = st.text_input("Name", value=selected_c.get("name", ""), key="alt_up_name")
                up_email = st.text_input("Email", value=selected_c.get("email", ""), key="alt_up_email")
                up_phone = st.text_input("Phone", value=selected_c.get("phone", ""), key="alt_up_phone")
                up_license = st.text_input("Driver License", value=selected_c.get("license_no", ""), key="alt_up_license")
                up_address = st.text_area("Address", value=selected_c.get("address", ""), key="alt_up_address")
...
# --- 3. FLEET INVENTORY ---
    ...
    with tab1:
        ...
        if st.button("🔄 Refresh Fleet List", key="alt_refresh_fleet"):
            st.rerun()
    ...
    with tab2:
        ...
        with st.form("add_vehicle_form_alt"):
            col_x, col_y = st.columns(2)
            with col_x:
                plate = st.text_input("Registration Plate No. (Unique)", placeholder="AP-01-XX-XXXX", key="alt_add_plate")
                model = st.text_input("Brand & Model Name", placeholder="Toyota Innova", key="alt_add_model")
            with col_y:
                vtype = st.selectbox("Vehicle Classification Type", ["Car", "Truck", "Van", "Motorcycle"], key="alt_add_type")
                rate = st.number_input("Standard Daily Rental Rate ($)", min_value=0.0, step=50.0, value=1500.0, key="alt_add_rate")
    ...
    with tab3:
        ...
        if isinstance(vehicles, list) and vehicles:
            ...
            selected_v_label = st.selectbox("Select Vehicle to Update", list(vehicle_options.keys()), key="alt_select_veh_up")
            selected_v = vehicle_options[selected_v_label]
            
            with st.form("update_vehicle_form_alt"):
                st.markdown(f"Updating specs for ID: **{selected_v['id']}** ({selected_v['plate']})")
                up_model = st.text_input("Brand & Model Name", value=selected_v.get("model", ""), key="alt_up_model")
                up_vtype = st.selectbox("Type", ["Car", "Truck", "Van", "Motorcycle"], index=["Car", "Truck", "Van", "Motorcycle"].index(selected_v.get("type", "Car")), key="alt_up_vtype")
                up_rate = st.number_input("Daily Rental Rate ($)", min_value=0.0, value=float(selected_v.get("rate", 0.0)), key="alt_up_rate")
                
                up_avail_str = st.selectbox("Set Availability Status", ["True", "False"], index=0 if selected_v.get("available") else 1, key="alt_up_avail_str")
...
# --- 4. RENTAL BOOKINGS ---
    ...
    with tab1:
        ...
        if isinstance(customers, list) and customers and isinstance(vehicles, list) and vehicles:
            ...
            if not avail_vehicles:
                ...
                selected_cust_label = st.selectbox("Select Booking Client", list(cust_options.keys()), key="alt_rent_cust")
                selected_veh_label = st.selectbox("Select Fleet Vehicle", list(veh_options.keys()), key="alt_rent_veh")
                ...
                with col_d1:
                    start_d = st.date_input("Start Date", value=date.today(), key="alt_rental_start_date")
                with col_d2:
                    end_d = st.date_input("End Date (Return Date)", value=date.today(), key="alt_rental_end_date")
                ...
                if st.button("Sign Rental Agreement", key="alt_sign_agreement"):
    ...
    with tab2:
        ...
        if st.button("🔄 Refresh Bookings Log", key="alt_refresh_rent"):
            st.rerun()
    ...
    with tab3:
        ...
        if isinstance(rentals, list) and rentals:
            ...
            selected_r_label = st.selectbox("Select Active Booking to Complete", list(rental_options.keys()), key="alt_select_rent_term")
            ...
            if st.button("Record Vehicle Return / Delete Booking", key="alt_return_btn"):
...
# --- 5. PAYMENT LEDGER ---
    ...
    with tab1:
        ...
        if isinstance(rentals, list) and rentals:
            ...
            selected_r_label = st.selectbox("Select Booking to Bill", list(rental_options.keys()), key="alt_select_rent_pay")
            selected_r = rental_options[selected_r_label]
            ...
            with st.form("capture_payment_form_alt"):
                st.markdown(f"Transaction billing details for **{selected_r.get('customer_name')}**")
                col_p1, col_p2 = st.columns(2)
                with col_p1:
                    cust_id_lbl = st.text_input("Customer ID (Auto-filled)", value=selected_r["customer_id"], disabled=True, key="alt_p_cust_id")
                    rental_id_lbl = st.text_input("Rental Booking ID (Auto-filled)", value=selected_r["id"], disabled=True, key="alt_p_rental_id")
                with col_p2:
                    p_type = st.selectbox("Select Payment Mode", ["Card", "UPI", "Cash"], key="alt_p_mode")
                    pay_date = st.date_input("Transaction Date", value=date.today(), key="alt_p_date")
                
                amt = st.number_input("Capture Payment Amount ($)", min_value=0.0, value=prefilled_amount, step=100.0, key="alt_p_amt")
    ...
    with tab2:
        ...
        if st.button("🔄 Refresh Financial Ledger", key="alt_refresh_pay"):
            st.rerun()
    ...
    with tab3:
        ...
        if isinstance(payments, list) and payments:
            ...
            selected_p_label = st.selectbox("Select Payment to Generate Receipt", list(pay_options.keys()), key="alt_select_pay_receipt")
...
# --- 6. MAINTENANCE LAB ---
    ...
    with tab1:
        ...
        if st.button("🔄 Refresh Maintenance Ledger", key="alt_refresh_maint"):
            st.rerun()
    ...
    with tab2:
        ...
        if isinstance(vehicles, list) and vehicles:
            ...
            if avail_veh:
                ...
                selected_v_label = st.selectbox("Select Vehicle for Service", list(veh_options.keys()), key="alt_select_veh_maint")
                selected_v = veh_options[selected_v_label]
                
                with st.form("dispatch_maintenance_form_alt"):
                    desc = st.text_area("Detailed Repairs Description (e.g. Engine tuneup, denting)", placeholder="Describe issue...", key="alt_m_desc")
                    cost = st.number_input("Estimated Service Cost ($)", min_value=0.0, step=100.0, value=500.0, key="alt_m_cost")
                    mdate = st.date_input("Service Date", value=date.today(), key="alt_m_date")
    ...
    with tab3:
        ...
        if isinstance(mrecords, list) and mrecords:
            ...
            selected_m_label = st.selectbox("Select Maintenance Record to Resolve", list(m_options.keys()), key="alt_select_m_resolve")
            ...
            if st.button("Complete Service / Delete Record", key="alt_m_resolve_btn"):
